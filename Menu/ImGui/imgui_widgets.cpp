...
void ImGui::LogBulletText(const char* fmt, ...)
{
...
//void ImGui::OriginalBulletText(const char* fmt, ...)
//{
//	va_list args;
//	va_start(args, fmt);
//	//BulletTextV(fmt, args);
...
bool ImGui::Tab(const char* label, bool selected)
{
	...
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 size = ImGui::CalcItemSize({ 8 + label_size.x,24 + 0 }, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
	...
	if (selected)
	{
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 0, bb.Min.y + 0), ImVec2(bb.Max.x + 0, bb.Max.y + 0), ImColor(79, 79, 79, 255), 0, 15);
	}
	else
	{
		if (!hovered)
	...
	if (selected)
	{
		...
		ImGui::RenderText(ImVec2(bb.Min.x + 0 + (style.ItemInnerSpacing.x), bb.Min.y + style.FramePadding.y + 2), label);
		ImGui::PopStyleColor();
	}
	else
	{
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(152 / 255.f, 152 / 255.f, 152 / 255.f, 255 / 255.f));
		ImGui::RenderText(ImVec2(bb.Min.x + 0 + (style.ItemInnerSpacing.x), bb.Min.y + style.FramePadding.y + 2), label);
		ImGui::PopStyleColor();
	}
	return pressed;
